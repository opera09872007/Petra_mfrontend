(function () {
  ...
  var open = function (editor) {
    var selectedNode = editor.selection.getNode(),
      ...
      isFootNotes =
        ...
        editor.dom.getAttrib(selectedNode, 'class') === 'fnoteWrap';
    ...
    if (isFootNotes) {
      name = selectedNode.name || decodeURIComponent(selectedNode.getAttribute('data-tips')) || '';
    }
    ...
    editor.windowManager.open({
      ...
      onSubmit: function (e) {
        var newfootnoteContent = e.getData().name,
          ...
          htmlTemplate =
            '<span class="fnoteWrap" id="#wk_ft{FOOTNOTE_INDEX}" contenteditable="false" data-tips=" ' +
            fixFootnoteContent +
            ' "><button type="button" class="fnoteBtn" data-content="">[{FOOTNOTE_INDEX}]</button></span>',
          ...
          var searchFor = function (_selector, el) {
            ...
            if (el) {
              ...
              el.children.forEach(function () {
                if (el) found = searchFor(_selector, this);
              });
              return found;
            }
            return null;
        ...
        if (nextFD.length) {
          ...
          if (selectIndex < totalCount) {
            // modify
            html = replaceTmpl(htmlTemplate, {
              FOOTNOTE_INDEX: $(totalFootNote[selectIndex - 1]).html(),
            });
          } else {
            // anywhere add
  ...
  var register$1 = function (editor) {
    editor.ui.registry.addToggleButton('footnotes', {
      ...
      onSetup: function (buttonApi) {
        return editor.selection.selectorChangedWithUnbind('span.fnoteWrap', buttonApi.setActive)
          .unbind;
      },
    });
  ...
  function Plugin() {
    global.add('footnotes', function (editor) {
      editor.ui.registry.addIcon('fnote', '<img src="/fn.png' + '">');
